...
/* Register the type of device and add the relevant classes to the body */
var device = {touch:false, mobile:false, over:'', out:'', retina:false, transitions:false, android:false};
function registerDevice()
{
    ...
    device.smoothScroll = true;
    var isMac = navigator.platform.toUpperCase().indexOf('MAC')>=0;
    if(!isMac)device.smoothScroll = false;
    ...
    if(!device.touch){document.getElementsByTagName('html')[0].className+=' desktop';}
}

registerDevice();

/* Smart animate, to animate using css transitions where possible */
(function($) {
    $.fn.smartAnimate = function(options, time,callback,vars)
    {
        ...
        this.setTransition = function(tgt,options,time,callback)
        {
            if(cssTransitions)
            {
                ...
                $(tgt).addClass('css-anim').css(options);
                clearTimeout($(tgt).data('css-timeout'));
                $(tgt).data('css-timeout', setTimeout(function(){that.setCSSTimes($(tgt),0); if(callback)callback(tgt,vars);},Math.max(time,50)));
            }else{
                $(tgt).stop(true).animate(options,time,function(){that.setCSSTimes($(tgt),0); if(callback)callback(tgt,vars);});
            }
        };
        ...
        $(this).each(function(){that.setTransition(this,options,time,callback);});
        
        return this;
    };
})(jQuery);

function getSuperTransition(options)
{
    var defaults = {noTranslate:false, x:null, y:null, units:'px', leftOffset:0, topOffset:0, z:null, rotateX:null, rotateY:null, rotateZ:null, scale:null, timing:'ease-out'};
    options = $.extend(defaults,options);
    
    var trnString = '';
    if(options.rotateX!==null)trnString+='rotateX('+options.rotateX+'deg) ';
    if(options.rotateY!==null)trnString+='rotateY('+options.rotateY+'deg) ';
    if(options.rotateZ!==null)trnString+='rotateY('+options.rotateZ+'deg) ';
    if(options.rotate!==null&&options.rotate!==undefined)trnString+='rotate('+options.rotate+'deg) ';
    if(options.scale!==null)trnString+='scale('+options.scale+') ';
    
    if(device.transforms&&device.animations&&!options.noTranslate)
    {
        if(options.x!==null)trnString+='translateX('+(parseInt(options.x,10)-parseInt(options.leftOffset,10))+options.units+') ';
        if(options.y!==null)trnString+='translateY('+(parseInt(options.y,10)-parseInt(options.topOffset,10))+options.units+') ';
        if(options.z!==null)trnString+='translateZ('+options.z+') ';
        return {transform:trnString, transitionTimingFunction:options.timing};
    }else{
        return {left:options.x+options.units, top:options.y+options.units, transform:trnString, transitionTimingFunction:options.timing};
    }
}

function nav() {
    var t = {},
    icon = {sIcoHeight:28, nvBtnWidth:65, nvBtnHeight:57, animTime:300, animTimeFast:200, navAnimTime:400};
    t.navButtonOver = function(item, options){
        $('.icon', item).smartAnimate(getSuperTransition($.extend(options, {}, {
            timing: 'cubic-bezier(0.175, 0.885, 0.32, 1.1)'
        })), 400);
    };
    
    t.navButtonOut = function(item){
        $('.icon', item).smartAnimate(getSuperTransition({
            x: 0,
            y: 0,
            timing: 'cubic-bezier(0.175, 0.885, 0.32, 1.1)'
        }), icon.navAnimTime);
    ...
    root.on('mouseenter','a',function(){$(this).addClass('hover');})
        .on('mouseleave','a',function(){$(this).removeClass('hover');})
        .on('mouseleave','.nv-btn-prev, .nv-btn-next, .nv-btn-db',function(){t.navButtonOut(this);})
       .on('mouseenter', '.nv-btn-prev',function(){t.navButtonOver(this,{x:-icon.nvBtnWidth});})
            .on('mouseenter', '.nv-btn-next',function(){t.navButtonOver(this,{x:icon.nvBtnWidth});})
            .on('mouseenter', '.nv-btn-db',function(){t.navButtonOver(this,{y:-icon.nvBtnWidth});})

}
var time = 3000,
    init, imgPlay, imgPlayInterval;

var play = function (index) {
    $('.main-bg').fadeOut();
    $('.main-bg-' + index).fadeIn();
};
...
imgPlay = function () {
    ...
    play(index);
    imgPlayInterval = setTimeout(imgPlay, time);
};

var txtPlay=(function(){
    var current=0;
    var scrollPaneNum=6;//变化的屏数
    var step=815;//一次变化的高度
    var duration=16;//一次变化需要的次数
    var scrollInterval;
    var focusPic=document.getElementById('text-wrap');
    ...
    return{
        turn:function(){
            current++;
            if(current>scrollPaneNum) {
                current=scrollPaneNum;
                $('.hist').fadeIn();
            }
            txtPlay.scroll();
        },
...
var bindPage = function () {
    $('.nv-btn-prev').on('click', function () {
        clearTimeout(imgPlayInterval);
        imgPlay();
        txtPlay.turn1();
        return false;
    });
    $('.nv-btn-next').on('click', function () {
        clearTimeout(imgPlayInterval);
        imgPlay();
        txtPlay.turn();
        return false;
...
>>>>>>> 2dd452df13396b9480e13fd2e95c39123ac70a76
