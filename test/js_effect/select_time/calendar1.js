...
/**
 * Calendar
 * @param canSelectPrev  可以选择以前默认为不可以
 ...
 */
function Calendar(canSelectPrev, beginYear, endYear, language, patternDelimiter, date2StringPattern, string2DatePattern) {
	this.canSelectPrev = canSelectPrev || false;
		...
		if (dateArray[i]){
			...
			tds[i].onclick = function () {
				if (calendar.dateControl){
					selectedDate = new Date(calendar.date.getFullYear(),
														calendar.date.getMonth(),
														this.innerHTML);
					if (calendar.canSelectPrev) {
						calendar.dateControl.value = selectedDate.format(calendar.date2StringPattern);
					}
					else if (today.getTime() <= selectedDate.getTime()) {
						calendar.dateControl.value = selectedDate.format(calendar.date2StringPattern);
					}
				}
				calendar.hide();
...
document.writeln('<div id="__calendarPanel" style="position:absolute;display:none;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:200px;height:216px;">');
