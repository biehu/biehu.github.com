...
function sogou()
{
	if (u(sogou_param) || u(sogou_param["ct"]) || u(sogou_param["pid"])) return;
	var sogou_ad_url = "";
	var ct = sogou_param["ct"];
	var pid = sogou_param["pid"];
	if (ct == "kwd")
		sogou_ad_url = "http://lu.sogou.com/";
	else if ((pid.substr(0,6)=="sohu__" || pid.substr(0, 7)=="sogou__" || pid=="sogoudh" || pid=="sogou_dh") && // lvl-2
			!(pid.substr(0,10)=="sohu__mail" && pid!="sohu__mail") && // sohu__mail* don't belong to lvl-2; sohu__mail does;
			Math.floor(Math.random()*1000000)<800000) // 80% traffic of lvl-2 goes to sas.sogou.com
		sogou_ad_url = "http://sas.sogou.com/";
	else
